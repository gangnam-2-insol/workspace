...
# 이력서 유사도 분석 관련 API
@app.post("/resume/register", response_model=dict)
async def register_resume(resume: ResumeData):
    """이력서를 등록하고 유사한 이력서들을 반환합니다."""
    try:
        result = await resume_service.register_resume(resume.model_dump())
        return result
...
@app.get("/resume/similar/{resume_id}")
async def get_similar_resumes(resume_id: str, limit: int = 5):
    """특정 이력서와 유사한 이력서들을 조회합니다."""
    try:
        similar_resumes = await resume_service.get_similar_resumes(resume_id, limit)
        return {"similar_resumes": similar_resumes}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
